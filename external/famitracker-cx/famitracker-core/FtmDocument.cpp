...
// chipmachine port: bumped 0x0430 -> 0x0440 so modules saved by the final
// official FamiTracker 0.4.x line load. The per-block readers below already
// handle the 0.4.4-0.4.6 block versions (PARAMS v6, etc.) and skip unknown
// blocks gracefully; only this top-level FILE_VER gate rejected them as TOONEW.
const unsigned int FILE_VER			= 0x0440;			// Current file version (4.40)
const unsigned int COMPATIBLE_VER	= 0x0100;			// Compatible file version (1.0)

#define FILE_VER_STR "4.40"
...
#define CMP(token) (strcmp(id, token) == 0)
		...
		{
			// chipmachine port: skip unknown blocks instead of failing the whole
			// load. FamiTracker 0.4.x added blocks this engine never knew (e.g.
			// "COMMENTS", and the VRC7/FDS/N163 sequence blocks); readBlock() has
			// already advanced the stream past the block's data, so dropping it
			// here just discards content we don't render and keeps the rest of the
			// module loadable. (Was: return false -> GENERALREADFAILURE.)
		}
	}
