...
public:
	...
	// chipmachine port: synchronous, thread-free playback for a pull-based host.
	// beginRender() arms playback (the work startTracker() does, minus the sink
	// thread); renderSamples() runs tracker frames on demand and copies up to
	// `sz` int16 mono samples into `buf`, returning how many it produced (a short
	// count means the song halted). isHalted() reports song end.
	void beginRender();
	core::u32 renderSamples(core::s16 *buf, core::u32 sz);
	bool isHalted() const { return m_bPlayerHalted; }
