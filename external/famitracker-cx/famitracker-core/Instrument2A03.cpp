...
bool CInstrument2A03::Load(Document *doc)
{
	...
	for (int i = 0; i < Octaves; i++)
	{
		for (int j = 0; j < 12; j++)
		{
			...
			// chipmachine port: FamiTracker 0.4.4+ (INSTRUMENTS block version 6)
			// stores a per-key DPCM delta-counter setting after the pitch byte.
			// This engine (v0.4.3-era) has no slot for it; consume the byte so the
			// rest of the block stays aligned. Leaving it unread was what made
			// readNew_instruments' "size < 256" assertion fire on 0x0440 modules.
			if (Version >= 6)
				doc->getBlockChar();
		}
	}
