...
#include <stdio.h>
#include <cmath>
// chipmachine port: boost::thread synchronisation replaced with the C++11
// standard library. The objects are still here so the upstream locking code
// compiles unchanged, but the player is driven synchronously (renderSamples)
// so they are effectively uncontended.
#include <mutex>
...
struct _soundgen_threading_t
{
	...
	std::mutex mtx_sink;
	std::mutex mtx_tracker;
	std::mutex mtx_rowframes;
	std::condition_variable cond_trackerhalt;
};
...
void SoundGen::timeCallback(core::u32 skip, void *data)
{
	...
	std::unique_lock<std::mutex> lock(sg->m_threading->mtx_tracker);
...
{
	std::lock_guard<std::mutex> lock(m_threading->mtx_running);
...
{
	{
		std::unique_lock<std::mutex> lock(m_threading->mtx_tracker);
...
// chipmachine port: arm playback synchronously. Mirrors startTracker() but
// without taking the running mutex or signalling the (absent) sink thread.
void SoundGen::beginRender()
{
	m_trackerActive = true;
	startPlayback();
}

// chipmachine port: pull-model render. Drains any audio left from the previous
// frame, then runs tracker frames -- each ProcessChannel/RefreshChannel pass
// feeds the APU, whose end-of-frame apuCallback pushes int16 mono samples into
// m_queued_sound -- until `buf` holds `sz` samples or the song halts. Returns
// the number of samples written (short on halt); the rowframe/timeidx/volume
// bookkeeping that requestSound() does for the GUI is intentionally skipped.
core::u32 SoundGen::renderSamples(core::s16 *buf, core::u32 sz)
{
	core::u32 off = 0;
	...
	while (sz != 0)
	{
		requestFrame();			// advances the tracker and renders one frame
		if (m_bPlayerHalted)	// set inside requestFrame() via TrackerController
			break;
