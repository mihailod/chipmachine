...
/* chipmachine: made static to avoid a link-time collision with eupplugin's
   own `frequencyTable` (both are private lookup tables; nothing outside this
   translation unit references these). */
static int32_t frequencyTable[SE_NROFFINETUNESTEPS][128];
static int16_t sineTab[256];
static uint8_t isStaticInit = 0;
