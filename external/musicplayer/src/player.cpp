...
public:
   ...
   // Write directly to a temporary file on your local home directory
    std::ofstream perf_log("/tmp/chipmachine_perf.log", std::ios::app);
    if (perf_log.is_open()) {
        perf_log << "[INIT] Probing song: " << songFile.string() << "\n";
        perf_log.flush();
    }
    ...
    for (const auto& plugin : musix::ChipPlugin::getPlugins()) {
        ...
        bool handle = plugin->canHandle(songFile.string());
        auto t1 = std::chrono::high_resolution_clock::now();
        
        double ms_handle = std::chrono::duration<double, std::milli>(t1 - t0).count();
        if (ms_handle > 1.0 && perf_log.is_open()) {
            perf_log << "[PERF] " << plugin->name() << "::canHandle took " << ms_handle << " ms\n";
            perf_log.flush();
        }
        ...
        if (handle) {
            try {
                ...
                if (auto* ptr = plugin->fromFile(songFile)) {
                    ...
                    if (perf_log.is_open()) {
                        perf_log << "[PERF] SUCCESS -> " << plugin->name() << "::fromFile took " 
