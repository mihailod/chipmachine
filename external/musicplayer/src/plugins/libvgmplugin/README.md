...
> **That file no longer contains Yamaha's ROM.** It used to: 8192 bytes, CRC32
> `23c9e0d8`, copyrighted firmware under no licence. It is now generated by
> [`gen_2608rom_synth.c`](gen_2608rom_synth.c) — see the header comment there
> for the full rationale.

### Replacing the rhythm ROM

`gen_2608rom_synth.c` synthesizes the six voices and encodes them to ADPCM-A in
the exact byte layout `YM2608_ADPCM_ROM_addr` expects, so no address table
changes. It is the companion to s98plugin's `fmgen/gen_rhythm_synth.c` — same
voice designs, but at the chip's own 18512 Hz with the ROM's sample counts, and
encoding rather than emitting PCM.

It emits **both** consumers of this ROM, which declare it differently:

```
cc -O2 gen_2608rom_synth.c -lm -o /tmp/g
/tmp/g ../../../../zxtune/3rdparty/vgm/emu/cores/fmopn_2608rom.h
/tmp/g ../dmfplugin/furnace/src/engine/platform/sound/rss.h nonstatic
```

Furnace uses the identical address layout (`set_start_end()` in its
`ymfm_opn.cpp`), so one generator serves both. There is deliberately **no
`.orig`** kept beside either file, unlike the `opnintf.c` patch — keeping one
would mean keeping the ROM.

Three things worth knowing about the encoder:

1. **It is closed-loop and exact.** It runs libvgm's own decoder
   (`ADPCMA_calc_chan`) and picks, per sample, the nibble landing nearest the
   target. Fed a representable signal it reproduces it exactly.
2. **Voices must not start abruptly.** The decoder begins with accumulator and
   step index at 0, where the largest jump is 30 units. A voice opening at peak
   is unencodable, so every voice gets a 24-sample (~1.3 ms) attack ramp.
3. **Never let the accumulator leave 12-bit range.** It *wraps* rather than
   saturating, and in a differential codec that displaces every later sample
   permanently. The real ROM's cymbal does exactly this — two nibbles early in
   its stream leave it with a **+477 DC offset (23% of full scale) for all
   643 ms**, which is why its raw RMS reads 490 against a true signal of 112.
   The encoder refuses any nibble that would wrap, so the generated voices carry
   no offset.

Residual encode error is under 2% on the tonal voices (BD, TOM) and about 15% on
the noise-based ones (SD, TOP, HH). That 15% is ADPCM-A's 4-bit quantization
floor on noise, not a synthesis or tuning problem — retuning barely moves it.
The error is noise added to noise, so it reads as slight extra grain.

Note the oscillator and filter frequencies are **lower** than the 55466 Hz
`gen_rhythm_synth.c` uses: Nyquist here is 9256 Hz, and content near it
alternates sign almost every sample, which is precisely what a differential
codec cannot track.
