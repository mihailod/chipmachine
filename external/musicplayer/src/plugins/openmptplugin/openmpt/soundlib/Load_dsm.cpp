...
// DSIK "old" Internal Format (DSM v1), magic "DSM"+0x10.
// [chipmachine local patch] libopenmpt only handles the newer RIFF/DSMF v2
// variant (ReadDSM below); the v1 format used by the Necros et al. tunes on
// modland is a fundamentally different, paragraph-pointer based layout that
// the upstream loader explicitly skips. This branch is ported from
// MilkyTracker's LoaderDSMv1 (BSD, milkyplay/LoaderDSM.cpp): same S3M-style
// effect encoding as v2 (so we reuse ConvertModCommand), but with a running
// XOR "obfuscation" on the pattern control bytes, notes shifted two octaves
// up (MilkyTracker octaveAdjust 0->24), and 8-bit signed samples reached via
// 16-byte paragraph pointers + "DSI"+0x10 sample chunks.
bool CSoundFile::ReadDSM(FileReader &file, ModLoadingFlags loadFlags)
{
	...
	{
		uint8 v1magic[4];
		if(file.ReadArray(v1magic)
			&& v1magic[0] == 'D' && v1magic[1] == 'S' && v1magic[2] == 'M' && v1magic[3] == 0x10)
		{
			return ReadDSMv1(file, loadFlags);
		}
		file.Rewind();
	}
...
/////////////////////////////////////////////////////////////////////
// DSIK "old" Internal Format (DSM v1) loader
// Ported from MilkyTracker LoaderDSMv1 (BSD). Entered from ReadDSM() when the
// file begins with "DSM"+0x10. [chipmachine local patch]

bool CSoundFile::ReadDSMv1(FileReader &file, ModLoadingFlags loadFlags)
{
	file.Rewind();
	file.Skip(4);  // "DSM" + 0x10

	char songName[28];
	file.ReadArray(songName);
	file.Skip(5);  // 4 unknown + 1 unknown

	const uint8 numChannels = file.ReadUint8();
	const uint8 numOrders   = file.ReadUint8();  // song length
	const uint8 numSamples  = file.ReadUint8();
	const uint8 numPatterns = file.ReadUint8();
	const uint8 speed       = file.ReadUint8();  // ticks/row (MilkyTracker "tempo")
	const uint8 tempo       = file.ReadUint8();  // BPM         (MilkyTracker "speed")
	const uint8 globalVol   = file.ReadUint8();
	file.Skip(4);

	if(numChannels < 1 || numChannels > 16 || numPatterns > 256)
		return false;
	...
	std::vector<uint16> insPtr(numSamples), patPtr(numPatterns);
	for(auto &p : insPtr) p = file.ReadUint16LE();
	for(auto &p : patPtr) p = file.ReadUint16LE();

	InitializeGlobals(MOD_TYPE_DSM, std::max<uint8>(numChannels, 1));
	m_modFormat.formatName = UL_("DSIK Format (old)");
	m_modFormat.type = UL_("dsm");
	m_modFormat.charset = mpt::Charset::CP437;

	m_songName = mpt::String::ReadBuf(mpt::String::maybeNullTerminated, songName);
	Order().SetDefaultSpeed(speed ? speed : 6);
	Order().SetDefaultTempoInt(tempo ? tempo : 125);
	m_nDefaultGlobalVolume = std::min(globalVol, uint8(64)) * 4u;
	if(!m_nDefaultGlobalVolume) m_nDefaultGlobalVolume = MAX_GLOBAL_VOLUME;

	ReadOrderFromArray(Order(), orders, numOrders, 0xFF, 0xFE);

	// Samples. The pattern instrument byte is the 1-based file instrument index,
	// so we keep sample index == instrument index (empty slots stay empty).
	m_nSamples = numSamples;
	for(SAMPLEINDEX smp = 1; smp <= numSamples; smp++)
	{
		const uint32 insOffset = static_cast<uint32>(insPtr[smp - 1]) * 16u;
		if(!insOffset || !file.Seek(insOffset))
			continue;
		if(file.ReadUint32LE() != 0x10495344u)  // "DSI" + 0x10
			continue;

		char sampleName[30];
		file.ReadArray(sampleName);
		file.Skip(1);  // zero terminator

		const uint8  byteOffset = file.ReadUint8();
		const uint16 wordOffset = file.ReadUint16LE();
		const uint32 sampleOffset = ((static_cast<uint32>(byteOffset) << 16) + wordOffset) * 16u;

		ModSample &sample = Samples[smp];
		sample.Initialize();
		const uint16 length    = file.ReadUint16LE();
		const uint16 loopStart = file.ReadUint16LE();
		const uint16 loopEnd   = file.ReadUint16LE();
		sample.nLength    = length;
		sample.nLoopStart = loopStart;
		sample.nLoopEnd   = loopEnd;
		sample.uFlags.set(CHN_LOOP, loopEnd > loopStart);
		sample.nC5Speed   = file.ReadUint16LE();
		sample.nVolume    = std::min(file.ReadUint8(), uint8(64)) * 4u;

		m_szNames[smp] = mpt::String::ReadBuf(mpt::String::maybeNullTerminated, sampleName);

		if((loadFlags & loadSampleData) && length && file.Seek(sampleOffset))
		{
	...
	// Patterns. Same nibble-flag packing as the v2 loader above, but the control
	// bytes are XOR-"obfuscated" with a running counter (data bytes advance the
	// counter without being XORed), and notes sit two octaves lower than v2.
	if(loadFlags & loadPatternData)
	{
		for(PATTERNINDEX pat = 0; pat < numPatterns; pat++)
		{
			...
			const uint32 patOffset = static_cast<uint32>(patPtr[pat]) * 16u;
			if(!patOffset || !file.Seek(patOffset))
				continue;

			uint16 packedSize = file.ReadUint16LE();
			uint32 remaining = (packedSize >= 2) ? (packedSize - 2u) : 0u;

			uint8 obf = 3;
			ROWINDEX row = 0;
			ModCommand dummy{};
			auto nextByte = [&]() -> uint8 { remaining--; return file.ReadUint8(); };

			while(remaining > 0 && row < 64)
			{
				const uint8 pi = nextByte() ^ obf++;
				if(pi == 0)
				{
					row++;
					continue;
				...
				const CHANNELINDEX chn = (pi & 0x0F);
				ModCommand &m = (chn < GetNumChannels()) ? *Patterns[pat].GetpModCommand(row, chn) : dummy;

				if((pi & 0x80) && remaining)
				{
					uint8 note = nextByte();
					obf++;
					if(note && note <= 12 * 9)
					{
						const int n = note + (11 + 24) + NOTE_MIN;  // v2 offset (+11) plus MilkyTracker octaveAdjust (+24)
						if(n <= NOTE_MAX) m.note = static_cast<ModCommand::NOTE>(n);
					}
				}
				if((pi & 0x40) && remaining)
				{
					m.instr = nextByte();
					obf++;
				}
				if((pi & 0x20) && remaining)
				{
					m.volcmd = VOLCMD_VOLUME;
					m.vol = std::min(nextByte(), uint8(64));
					obf++;
				}
				if((pi & 0x10) && remaining >= 2)
				{
					const uint8 command = nextByte();
					const uint8 param = nextByte();
					obf += 2;
					ConvertModCommand(m, command, param);
				}
			}
