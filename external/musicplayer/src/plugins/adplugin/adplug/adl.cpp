...
bool CadlPlayer::load(const std::string &filename, const CFileProvider &fp)
{
	...
	//
	// Westwood .snd files (e.g. Eye of the Beholder, Legend of Kyrandia) use the
	// same headerless ADL format under a different, generic extension.
	if (!fp.extension(filename, ".adl") && !fp.extension(filename, ".snd"))
		return false;
