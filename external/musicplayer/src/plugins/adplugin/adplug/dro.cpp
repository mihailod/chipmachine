...
bool CdroPlayer::load(const std::string &filename, const CFileProvider &fp)
{
	...
	if (version == 0x10000) {
		...
		// DRO v2.0 -- a different layout handled by Cdro2Player (dro2.cpp).
		fp.close(f);
		return false;
	} else {
		// Pre-1.0 DRO capture (DOSBox 0.63): there is NO version field, so the
		// four bytes just read are really the length in milliseconds. Layout is
		// "DBRAWOPL" + lengthMs(4) + lengthBytes(4) + hwType(1) + OPL data, and
		// the command-stream encoding is identical to v1.0, so the same render
		// path applies once data/iLength are populated.
		this->iLength = f->readInt(4); // length of the OPL data, in bytes
		// remaining bytes after the single hardware-type byte that follows
		if (this->iLength < 3 ||
			this->iLength > fp.filesize(f) - f->pos() - 1) {
			fp.close(f);
			return false;
		}
		f->ignore(1);	// hardware type (0=OPL2, 1=Dual OPL2, 2=OPL3) - ignored
		this->data = new uint8_t[this->iLength];
