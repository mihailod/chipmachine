...
bool CimfPlayer::load(const std::string &filename, const CFileProvider &fp)
{
  ...
  // Some real-world rips (e.g. the "ADLIB"-header ".adlib" loader tunes) declare
  // a song length that runs to EOF but isn't a multiple of the 4-byte event
  // size. AdPlug <= 2.3.2 played these by truncating the trailing partial event;
  // 2.4's stricter `song_size & 3` check below rejects them outright. Preserve
  // the old leniency for the to-EOF case so those tunes keep playing.
  if ((song_size & 3) && song_size + hdr_size >= file_size) { song_size &= ~3UL; }
