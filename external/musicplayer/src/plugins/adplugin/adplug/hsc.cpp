...
bool ChscPlayer::load(const std::string &filename, const CFileProvider &fp)
{
  ...
  // load patterns: each cell is a 2-byte hscnote, so a pattern is 64*9*2 bytes
  // on disk (1152, matching total_patterns_in_hsc above). The old code read
  // only 50*64*9 = 28800 bytes -- half the array -- leaving every pattern from
  // index 25 up empty, so any song arranging high patterns played silent.
  int pattern_bytes = total_patterns_in_hsc * 1152;
  if (pattern_bytes > (int)sizeof(patterns)) pattern_bytes = (int)sizeof(patterns);
