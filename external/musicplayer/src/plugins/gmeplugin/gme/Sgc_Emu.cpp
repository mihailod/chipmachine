...
{
	Sgc_Emu::header_t header_;

	Sgc_File() { set_type( gme_sgc_type ); }

	blargg_err_t load_( Data_Reader& in )
	{
		blargg_err_t err = in.read( &header_, Sgc_Emu::header_t::size );
		if ( err )
			return (err == in.eof_error ? gme_wrong_file_type : err);
		if ( !header_.valid_tag() )
			return gme_wrong_file_type;
		set_track_count( header_.song_count );
		return 0;
	}

	blargg_err_t track_info_( track_info_t* out, int ) const
	{
		copy_sgc_fields( header_, out );
		return 0;
	}
};
...
static gme_type_t_ const gme_sgc_type_ = { "Z80 PSG", 0, &new_sgc_emu, &new_sgc_file, "SGC", 1 };
extern gme_type_t const gme_sgc_type = &gme_sgc_type_;
