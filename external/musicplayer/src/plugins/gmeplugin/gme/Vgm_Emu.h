...
public:
	...
	struct header_t
	{
		...
		byte data_offset [4];
		byte unused2 [8];
		// Extended VGM 1.51+ header fields. header_size stays 0x40 (the original
		// header end and start-of-data anchor); these only let us name the chip
		// clocks that live past it. Read them only after a version/size check.
		byte unused3 [0x74 - 0x40]; // 0x40..0x74: clocks for chips we don't emulate
		byte ay8910_rate [4];      // 0x74
		byte ay8910_type;          // 0x78
		byte ay8910_flags;         // 0x79
		byte ym2203_ay_flags;      // 0x7A
		byte ym2608_ay_flags;      // 0x7B
	};
