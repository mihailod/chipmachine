...
public:
	...
	// GBR (the older Game Boy rip format) is decoded by this same emulator;
	// the 0x20-byte GBR header is rewritten into the GBS header_t at load time
	// (see load_()). new_gbr_emu() in Gbs_Emu.cpp sets this flag.
	void set_gbr_mode( bool b ) { gbr_mode_ = b; }
...
protected:
	...
	blargg_err_t load_gbr_( Data_Reader& ); // GBR header rewrite, see Gbs_Emu.cpp
...
private:
	...
	header_t header_;
	bool gbr_mode_ = false;
	int gbr_default_bank_ = 1;
