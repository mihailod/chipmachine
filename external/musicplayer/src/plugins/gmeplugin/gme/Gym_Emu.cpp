...
static blargg_err_t check_header( byte const* in, long size, int* data_offset = 0 )
{
	...
	if ( memcmp( in, "GYMX", 4 ) == 0 )
	{
		...
		// Packed (zlib-compressed) GYMX bodies are unpacked at load time;
		// see unpack_gym_body(). Without zlib we can't decode them.
#if !defined (HAVE_ZLIB_H)
...
#if defined (HAVE_ZLIB_H)
// A packed GYMX file stores a plain 428-byte header followed by a raw zlib
// stream; the header's "packed" field holds the uncompressed body length.
// Inflate that body into out (in==compressed body, out_size==uncompressed len).
static blargg_err_t unpack_gym_body( byte const* in, long in_size,
		long out_size, blargg_vector<byte>& out )
{
	if ( out_size <= 0 || in_size <= 0 )
		return "Couldn't unpack GYM file";
	RETURN_ERR( out.resize( (size_t) out_size ) );
	uLongf dest_len = (uLongf) out_size;
	if ( uncompress( (Bytef*) out.begin(), &dest_len,
			(Bytef const*) in, (uLong) in_size ) != Z_OK )
		return "Couldn't unpack GYM file";
	if ( (long) dest_len != out_size )
		RETURN_ERR( out.resize( (size_t) dest_len ) );
	return 0;
}
#endif
...
struct Gym_File : Gme_Info_
{
	...
	byte const* file_end;
	int data_offset;
	blargg_vector<byte> packed_data; // inflated sequence when GYMX is packed
	...
	blargg_err_t load_mem_( byte const* in, long size )
	{
		...
		file_end   = in + size;
		data_offset = 0;
		RETURN_ERR( check_header( in, size, &data_offset ) );
#if defined (HAVE_ZLIB_H)
		if ( data_offset )
		{
			long packed_size = get_le32(
					((Gym_Emu::header_t const*) in)->packed );
			if ( packed_size )
				RETURN_ERR( unpack_gym_body( in + data_offset,
						size - data_offset, packed_size, packed_data ) );
		}
#endif
		return 0;
	}
	...
	{
		byte const* begin = &file_begin [data_offset];
		byte const* end   = file_end;
		if ( packed_data.size() )
		{
			begin = packed_data.begin();
			end   = packed_data.end();
		}
		long length = gym_track_length( begin, end );
...
#if defined (HAVE_ZLIB_H)
		long packed_size = get_le32( header_.packed );
		if ( packed_size )
		{
			RETURN_ERR( unpack_gym_body( in + offset, size - offset,
					packed_size, packed_data ) );
			data     = packed_data.begin();
			data_end = packed_data.end();
			return 0;
		}
#endif
		data     = in + offset;
