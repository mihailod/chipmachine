...
{
	// NOTE: end_time can legitimately be a clock or two before last_time here.
	// The VGM player derives PSG write times via to_blip_time() (fm/blip time
	// bases, "timing is working mostly by luck" in Vgm_Emu_Impl) while frame
	// ends come from Blip_Buffer::count_clocks(); the two rounding bases drift
	// across frame boundaries. A backward step is a sub-sample no-op, so just
	// skip it rather than abort (the old require() crashed on long 32X VGMs).
