...
void Vgm_Emu::set_voice( int i, Blip_Buffer* c, Blip_Buffer* l, Blip_Buffer* r )
{
	...
	// Route the AY (mono, not a PSG voice) into the shared center buffer. Every
	// voice in a Stereo_Buffer hands back the same center, and it's only null
	// when that voice is muted, so latch it from any unmuted voice.
	if ( ay_enabled && !uses_fm && c )
		ay.output( c );
}
...
void Vgm_Emu::mute_voices_( int mask )
{
	...
	// The AY isn't part of the Classic_Emu voice system. On the FM path the mixed
	// output is blip_buf (read by Dual_Resampler), so route it there. On the
	// classic PSG path the read buffer is the Stereo_Buffer, whose shared center
	// channel is what buf->end_frame() actually drains -- routing the AY into
	// blip_buf there would let it fill unbounded and overflow. That center buffer
	// isn't reachable here (buf is private to Classic_Emu), so set_voice() -- which
	// Classic_Emu hands the per-voice buffers -- routes the AY for the PSG path.
	if ( ay_enabled && uses_fm )
		ay.output( &blip_buf );
	if ( uses_fm )
	{
...
blargg_err_t Vgm_Emu::load_mem_( byte const* new_data, long new_size )
{
	...
	// psg rate
	long raw_psg = get_le32( h.psg_rate );
	psg_dual = ( raw_psg & 0x40000000 ) != 0;
	psg_t6w28 = ( raw_psg & 0x80000000 ) != 0;
	psg_rate = raw_psg & 0x0FFFFFFF;

	// AY-3-8910 (added in VGM 1.51): Vectrex / ZX Spectrum rips use it instead
	// of the SN76489 PSG. The blip-time domain is clocked at psg_rate, and the
	// AY oscillator periods are reckoned against that same blip clock, so for an
	// AY-only tune (no SN PSG) clock the domain at the AY rate to get the right
	// pitch. (A tune with both chips at different rates is not handled.)
	ay_enabled = false;
	long ay_rate = 0;
	if ( get_le32( h.version ) >= 0x151 && new_size >= 0x78 )
	{
		ay_rate = get_le32( h.ay8910_rate ) & 0x3FFFFFFF;
		if ( ay_rate )
			ay_enabled = true;
	}

	if ( !psg_rate )
		psg_rate = ay_enabled ? ay_rate : 3579545;
