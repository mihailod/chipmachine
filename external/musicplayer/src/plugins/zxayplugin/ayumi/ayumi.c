...
/* Local vendoring change: `static`. Upstream leaves this one internal helper
   with external linkage (every other one here is already static), and the name
   collides at link time with musxplugin's envelope.c. */
static int update_envelope(struct ayumi* ay) {
