...
{
	rate = 8000;
	// Prefer external rhythm .WAV samples if a path provides them; otherwise
	// fall back to the built-in OPNA rhythm sample ROM so hardware-rhythm
	// drums play with no runtime/secondary file. SetRate() below recomputes
	// rhythm[].step for the real output rate.
	if (!LoadRhythmSample(path))
		LoadEmbeddedRhythm();
...
// ---------------------------------------------------------------------------
//	内蔵リズム音源 (synthesized OPNA rhythm voices) を読みこむ
//	Built-in rhythm samples, synthesized from scratch -- see
//	opna_rhythm_rom.cpp, generated by gen_rhythm_synth.c. These are NOT
//	derived from Yamaha's YM2608 ADPCM-A rhythm ROM, which is not present in
//	this repository. Used when no external rhythm .WAV set is provided, so
//	FMP/S98 drums work out of the box; a user supplying the real 2608_*.WAV
//	set still gets authentic drums via LoadRhythmSample() above.
//
bool OPNA::LoadEmbeddedRhythm()
{
	extern const int opna_rhythm_present;
	extern const int opna_rhythm_rate;
	extern const int opna_rhythm_len[6];
	extern const int16* const opna_rhythm_pcm[6];

	if (!opna_rhythm_present)
		return false;

	for (int i = 0; i < 6; i++)
	{
		if (rhythm[i].sample) delete[] rhythm[i].sample;
		int len = opna_rhythm_len[i];
		rhythm[i].sample = new int16[len];
		if (!rhythm[i].sample)
		{
			for (int j = 0; j < 6; j++)
			{
				if (rhythm[j].sample) delete[] rhythm[j].sample;
				rhythm[j].sample = 0;
			}
			return false;
		}
		memcpy(rhythm[i].sample, opna_rhythm_pcm[i], len * sizeof(int16));
		rhythm[i].rate = opna_rhythm_rate;
		rhythm[i].step = rhythm[i].rate * 1024 / rate;
		rhythm[i].pos  = rhythm[i].size = len * 1024;
	}
	return true;
}
