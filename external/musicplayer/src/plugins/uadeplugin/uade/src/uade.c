...
{
  // Total size allocated for the raw buffer
  ...
  // Explicitly calculate the exact space left in the 'space' array object
  // Total array size minus the offsets we already jumped over
  size_t max_string_len = sizeof(space) - (sizeof(struct uade_msg) + 8);
  
  // Safely copy using the verified remaining size boundaries
  strlcpy((char *) um->data + 8, reason, max_string_len);
  
  // Set size explicitly based on what actually fits in the destination buffer
  um->size = 8 + strlen((char *) um->data + 8) + 1;
