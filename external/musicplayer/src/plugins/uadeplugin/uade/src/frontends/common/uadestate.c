				       ...
				       struct uade_state *state)
{
	...
	/*
	 * Probe-miss is non-fatal: players (e.g. Special-FX_ST for .doda) ask
	 * for optional auxiliary files by names that are often binary garbage.
	 * The lookup fails and the player falls back and plays fine. Printing
	 * "File not found <garbage>" to stderr only interleaves with cmtest's
	 * stdout and makes passing fixtures look like errors. Stay silent.
	 */
	return NULL;
    }
			      ...
			      struct uade_state *state)
{
	...
	/*
	 * chipmachine local patch: tolerate UADE_EVENT_EAGAIN before the
	 * playloop. A module whose player needs more than ~3 seconds of rendered
	 * audio before it reports subsong info (e.g. MusicMaker4 tunes with a long
	 * silent intro like "tribal dance.mm4", or some David Whittaker .dw) trips
	 * uadecore's "involuntary audio output start" safety valve, which begins
	 * streaming audio and yields a UADE_EVENT_EAGAIN ("call again") *before*
	 * UADE_EVENT_SUBSONG_INFO arrives. EAGAIN is benign by definition -- it
	 * just asks us to pump uade_get_event() again -- so the original code
	 * treating it as a fatal default-case error wrongly failed those tunes to
	 * load (uade_play() returned -1). Loop again until SUBSONG_INFO/SONG_END,
	 * bounded by eagain_budget so a pathological core that never reports
	 * subsong info still fails instead of spinning forever. Re-apply on
	 * uade revendor.
	 */
	int eagain_budget = 10000;
	...
	while (1) {
		...
		case UADE_EVENT_EAGAIN:
			if (--eagain_budget < 0) {
				uade_warning("Too many UADE_EVENT_EAGAIN before playloop; giving up.\n");
				goto fatalerror;
			}
			break;
		case UADE_EVENT_MESSAGE:
