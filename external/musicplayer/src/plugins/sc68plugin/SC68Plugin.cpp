...
bool SC68Plugin::canHandle(const std::string& name)
{
    ...
    if (ext == "sc68" || ext == "sndh") { return true; }
    if (ext == "snd") {
        if (utils::File::exists(name)) {
            try {
                utils::File f{name};
                if (f.getSize() >= 12) {
                    uint8_t header[128];
                    auto size = f.read(header, sizeof(header));
                    return sc68_verify_mem(header, static_cast<int>(size)) >= 0;
                }
...
std::set<std::string> SC68Plugin::getSupportedExtensions() const
{
