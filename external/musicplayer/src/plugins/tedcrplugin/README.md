...
corpus** before the removal: of 1,051 files where a comparison against the engine
was meaningful, **1,036 matched at cosine ≥ 0.90**, median 0.990, with six tunes
in 1,225 (**0.5%**) genuinely failing.
...
Same method as the sound core: a loop of known cycle count toggling the volume,
rendered through the engine, with the clock read back out of the resulting
frequency. The numbers decompose exactly:

* a raster line is **114 master cycles**, of which **5** are stolen for DRAM
  refresh, leaving 109 for the CPU
* the CPU runs at **half rate over 204 display lines** when `$FF06` bit 4 is set,
  and at full rate otherwise
* so a frame is 312 × 109 = **34,008 cycles with the screen off** — which is what
  was measured, to the cycle — and 22,890 with it on
* sound counters run at master/8, timers at master/2

Everything is driven in master cycles for that reason; CPU cycles are worth one
or two of them depending on the display state.
...
| silent under tedplay as well | 99 |
| **compared** | **1,051** |
| cosine ≥ 0.90 | **1,036** (98.6%) |
| between 0.75 and 0.90 | 0 |
| below 0.75 | 15 |

median 0.990, mean 0.972, 5th percentile 0.959.
...
* **The 0.5% tail.** Six tunes render silent: `csabo_quadrillion`,
  `grumskiz_the_economic_universe`, `hrh_mozaik_hrh`, `kent_robert_torpedo_run`,
  `ko-ko_aliens_demo`, `tfss_kutato`. They share an unresolved signature — the
  player runs and writes frequency registers about once per frame but never
  writes `$FF11`, so the channels are never enabled. Two more
  (`luca_2048_octaluxe`, `luca_the_it_crowd`) play correctly but start nine
  seconds late, skipping an intro. Note the reference engine is gone, so
  diagnosing these now means reading the tunes rather than diffing traces.
* **Subtune selection** — the corpus selects between tunes by keypress; `$FF08`
  currently reports nothing held down and `pressKey()` is a stub without a real
  keyboard matrix. The plugin reports one song per file.
* **SID card** — tedplay enables one unconditionally (`Tedmem.cpp:123`), so its
  unattributed `Sid.cpp` is mixed into every render. Only 11 of 148 sampled files
  touch `$FD40`–`$FD5F`, and 6 of those do exactly 4,096 writes, which is a blind
  `$FDxx` sweep rather than a SID player. This machine has no SID at all; cSID
  (WTFPL) is already in the tree if Plus/4 SID-card tunes ever turn up.
