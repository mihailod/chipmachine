...
//chipmachine local patch: ares includes nall through <ares/ares.hpp>; this build
//supplies only the slice the core uses. See PROVENANCE.md.
#include "nall_compat.hpp"
...
struct V30MZ {
  ...
  //chipmachine local patch: serialization.cpp and disassembler.cpp are not
  //vendored (they need nall's serializer and string), so their declarations are
  //removed here. Nothing else in the core calls them. See PROVENANCE.md.
