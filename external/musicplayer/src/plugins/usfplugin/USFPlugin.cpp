...
std::set<std::string> USFPlugin::getSupportedExtensions() const
{
    return supported_ext;
}

std::vector<std::string> USFPlugin::getSecondaryFiles(const std::string& name)
{
    // .miniusf references a shared .usflib via PSF "_lib" tags; fetch it
    // alongside when streaming so the loader finds it on a clean machine.
