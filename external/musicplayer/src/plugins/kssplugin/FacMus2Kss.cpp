// FacMus2Kss.cpp -- convert a FAC SoundTracker (MSX) ".MUS" song into an
// in-memory KSS image our vendored libkss can play.
//
// FAC SoundTracker (Federation Against Commodore, 1990/1991) is a PSG + sampled
// drumkit MSX tracker. A ".MUS" is the song (16391 bytes = 7-byte MSX BSAVE
// header + a 0x4000 page); its percussion lives in companion ".SM1"/".SM2"
// drumkit sample banks named inside the song header (or "NO DRUMS"). The
// faithful playback path is to run FAC's own Z80 replay routine on an emulated
// Z80+PSG, so we wrap song+drumkits+player into a KSS and hand it to libkss.
//
// Ported from rePlayer's ReplayNEZplugMUS2KSS.cpp (Arnaud Neny), itself a port
// of Juergen Wothke's mus_converter.js (https://bitbucket.org/wothke/webnez).
// The embedded fac_player[] is FAC SoundTracker 2.0's own replay routine
// (C)1990/1991 FAC -- scene freeware, redistributed verbatim like the MGSDRV
// driver blob libkss already embeds. See FacMus2Kss.h and the credits/LEGAL.

#include "FacMus2Kss.h"

#include <cstring>

namespace musix {
namespace fac {

// FAC ".MUS"/drumkit pages are a 7-byte MSX BSAVE header (0xFE, load 0x8000,
// end 0xBFFF, exec) followed by a 0x4000 (16 KB) RAM page.
constexpr size_t kBsaveHeader = 7;
constexpr size_t kPage = 0x4000;
constexpr size_t kMusSize = kBsaveHeader + kPage; // 16391

// Header fields live near the end of the page (absolute file offsets, i.e.
// including the 7-byte BSAVE header).
constexpr size_t kOffDrumkit = 0x3f8b; // 8-char drumkit base name, or "NO DRUMS"
constexpr size_t kOffVersion = 0x3f93; // 2 = SoundTracker, 3 = "Pro" (scrambled)
constexpr size_t kOffTitle = 0x3fad;
constexpr size_t kOffArtist = 0x3fd6;

// "Pro" songs (version byte == 3) store the page XOR-scrambled with an
// incrementing key; unscramble in place before use, then mark it as V2.0.
static void ConvertProData(uint8_t* data)
{
    uint8_t value = 0x00;
    for (size_t i = 0; i < 0x3F00; i++)
    {
        data[7 + i] ^= value;
        value = (value + 1) & 0xff;
    }
    data[0x3f93] = 2; // make it SoundTracker V2.0
}

// True when the song declares it uses no drumkit ("NO DRUMS" at 0x3f8b).
static bool NoMusDrums(const uint8_t* data)
{
    const char str[] = "NO DRUMS";
    for (size_t i = 0; i < sizeof(str) - 1; i++)
        if (data[0x3f8b + i] != str[i])
            return false;
    return true;
}

static uint8_t* CopyData(uint8_t* data, const void* src, size_t size)
{
    memcpy(data, src, size);
    return data + size;
}

// FAC SoundTracker 2.0 player routine ((C)1990/1991 FAC, scene freeware).
static const uint8_t fac_player[] = {	// FAC Soundtracker 2.0 player routine
            
        };

bool IsFacMus(const uint8_t* data, size_t size)
{
    // FAC ".MUS" is an MSX BSAVE image of one 16 KB page: 0xFE marker, load
    // address 0x8000, end 0xBFFF. The song proper is the first 0x4007 bytes;
    // some FAC saves append a little trailing padding, so real files run 16391
    // (the usual size), 16392, 16512 or 16513 bytes on Modland. Accept anything
    // at least the page size rather than an exact match -- a strict ==16391
    // gate silently dropped ~20% of the corpus. The 7-byte BSAVE wrapper (0xFE,
    // load 0x8000, end 0xBFFF) keeps the match specific. "Pro" songs scramble
    // the page but keep this header. (A short/truncated .mus below 16391 -- e.g.
    // the lone 16384-byte QUASAR51.MUS -- is rejected: too short to convert.)
    return size >= kMusSize && data[0] == 0xFE && data[1] == 0x00 &&
           data[2] == 0x80 && data[3] == 0xFF && data[4] == 0xBF;
}

std::string FacDrumkitBaseName(const uint8_t* data, size_t size)
{
    if (size < kMusSize || NoMusDrums(data)) { return {}; }
    char name[9];
    memcpy(name, data + kOffDrumkit, 8);
    name[8] = 0;
    // Trim trailing spaces/NULs from the 8-char field.
    int end = 7;
    while (end >= 0 && (name[end] == ' ' || name[end] == 0)) { --end; }
    return std::string(name, name + end + 1);
}

bool FacMusToKss(const uint8_t* input, size_t size,
                 const CompanionReader& openCompanion,
                 std::vector<uint8_t>& kssOut, std::string& title,
                 std::string& artist)
{
    if (size < kMusSize) { return false; }

    // Work on a private copy so we can unscramble "Pro" songs without touching
    // the caller's buffer.
    std::vector<uint8_t> work(input, input + size);
    uint8_t* data = work.data();
    bool isPro = data[kOffVersion] == 3;
    if (isPro) { ConvertProData(data); }

    static const uint8_t kssHeadNoDrums[] = {
                  0x4B, 0x53, 0x53, 0x58, 0x00, 0x40, 0xFF, 0xFF, 0x10, 0x80, 0x38, 0x00, 0x00, 0x00, 0x10, 0x08,
                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 // to force only 1 subsong
                };

    static const uint8_t kssHeadDrums[] = {
                        0x4B, 0x53, 0x53, 0x58, 0x00, 0x00, 0xFF, 0xFF, 0x10, 0xC0, 0x38, 0x00, 0x00, 0x00, 0x10, 0x08,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 // to force only 1 subsong
                    };

    static const uint8_t noDrumPatch[] = {
                  0x21, 0x00, 0x80, 0x11, 0x00, 0xD0, 0x01, 0x96, 0x08, 0xED, 0xB0, 0x21, 0x47, 0xD0, 0x11, 0x20,
                  0x00, 0x01, 0x06, 0x00, 0xED, 0xB0, 0x3E, 0xC3, 0x21, 0x9A, 0xFD, 0x32, 0x38, 0x00, 0x22, 0x39,
                  0x00, 0x3E, 0x40, 0x32, 0xA9, 0xD0, 0x32, 0xC8, 0xD0, 0x3E, 0x7F, 0x32, 0xAC, 0xD0, 0x32, 0xE8,
                  0xD0, 0xCD, 0xA1, 0xD0, 0xC3, 0x06, 0xD0, 0x7C, 0x92, 0xC0, 0x7D, 0x93, 0xC9
                };

    static const uint8_t drumPatch[] = {
                        0x21, 0x00, 0xC0, 0x11, 0x00, 0xD0, 0x01, 0x96, 0x08, 0xED, 0xB0, 0xAF, 0x32, 0x48, 0xD8, 0xCD,
                        0x03, 0xD0, 0x3E, 0x40, 0x32, 0x48, 0xD8, 0xCD, 0x03, 0xD0, 0x21, 0x00, 0x00, 0x11, 0x01, 0x00,
                        0x01, 0xFF, 0x3F, 0x36, 0xC9, 0xED, 0xB0, 0x21, 0x6D, 0xD0, 0x11, 0x20, 0x00, 0x01, 0x06, 0x00,
                        0xED, 0xB0, 0x3E, 0xC3, 0x21, 0x9A, 0xFD, 0x32, 0x38, 0x00, 0x22, 0x39, 0x00, 0x21, 0x00, 0x80,
                        0x11, 0x00, 0x40, 0xD5, 0xC1, 0xED, 0xB0, 0x3E, 0x40, 0x32, 0xA9, 0xD0, 0x32, 0xC8, 0xD0, 0x3E,
                        0x7F, 0x32, 0xAC, 0xD0, 0x32, 0xE8, 0xD0, 0xCD, 0xA1, 0xD0, 0xC3, 0x06, 0xD0, 0x7C, 0x92, 0xC0,
                        0x7D, 0x93, 0xC9
                    };

    if (NoMusDrums(data))
    {
        // Song-only KSS: [header][song page][player], with the player's bank
        // loader neutralised by noDrumPatch.
        kssOut.resize(sizeof(kssHeadNoDrums) + kPage + sizeof(fac_player));
        uint8_t* p = CopyData(kssOut.data(), kssHeadNoDrums, sizeof(kssHeadNoDrums));
        p = CopyData(p, data + kBsaveHeader, kPage);
        // p now points at the start of the just-to-be-copied player image; the
        // patches below poke into that player region.
        CopyData(p, fac_player, sizeof(fac_player));
        p[0xC] = 0;
        p[0xA3] = 0x7F;
        CopyData(p + 0x10, noDrumPatch, sizeof(noDrumPatch));
    }
    else
    {
        // A drummed song needs both <DRUMKIT>.SM1 and .SM2 sample banks next to
        // it. Without them we can't render the percussion, so decline (the song
        // would otherwise play with broken/garbage drums).
        std::string base = FacDrumkitBaseName(data, size);
        if (base.empty()) { return false; }
        std::vector<uint8_t> sm1, sm2;
        if (!openCompanion(base + ".SM1", sm1) || sm1.size() < kMusSize) {
            return false;
        }
        if (!openCompanion(base + ".SM2", sm2) || sm2.size() < kMusSize) {
            return false;
        }

        kssOut.resize(sizeof(kssHeadDrums) + 2 * kPage + kPage + sizeof(fac_player));
        uint8_t* p = CopyData(kssOut.data(), kssHeadDrums, sizeof(kssHeadDrums));
        p = CopyData(p, sm1.data() + kBsaveHeader, kPage);
        p = CopyData(p, sm2.data() + kBsaveHeader, kPage);
        p = CopyData(p, data + kBsaveHeader, kPage);
        // p now points at the start of the player image.
        CopyData(p, fac_player, sizeof(fac_player));
        p[0xC] = 0;
        CopyData(p + 0x10, drumPatch, sizeof(drumPatch));
    }

    // libkss's VM_init_memory copies exactly `load_len` bytes from our buffer
    // into emulated RAM, clamping only to 0x10000-load_adr -- NOT to the actual
    // data size (unlike the NEZplug++ VM rePlayer targets). The verbatim rePlayer
    // headers declare load_len = 0xFFFF, so libkss would read thousands of bytes
    // of heap garbage past the end of our buffer into high RAM -- precisely the
    // FAC player's zero-initialised work area -- causing intermittent silence
    // that depends on heap contents. Set load_len (KSSX header bytes 6-7) to the
    // real payload size so the copy stops at our data and the remaining RAM keeps
    // the zero fill libkss already applied.
    uint32_t payload = static_cast<uint32_t>(kssOut.size() - 32); // minus header
    kssOut[6] = static_cast<uint8_t>(payload & 0xff);
    kssOut[7] = static_cast<uint8_t>((payload >> 8) & 0xff);

    // Title/artist are fixed-width fields near the end of the page (one byte
    // shorter in "Pro" songs). Trim trailing spaces/NULs.
    auto field = [&](size_t off) {
        size_t len = isPro ? 36 : 41;
        if (off + len > size) { return std::string{}; }
        std::string s(reinterpret_cast<const char*>(data + off), len);
        size_t e = s.find_last_not_of(std::string(" \0", 2)); // space + NUL
        return e == std::string::npos ? std::string{} : s.substr(0, e + 1);
    };
    title = field(kOffTitle);
    artist = field(kOffArtist);
    return true;
}

} // namespace fac
} // namespace musix
