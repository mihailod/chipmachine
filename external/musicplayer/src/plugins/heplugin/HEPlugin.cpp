...
bool HEPlugin::canHandle(const std::string& name)
{
    ...
    // Check for SoundFactory in path as a quick skip
    if (utils::toLower(name).find("/soundfactory") != std::string::npos) {
        return false;
    }

    // PSF files MUST start with "PSF" magic bytes (0x50 0x53 0x46)
    FILE* f = fopen(name.c_str(), "rb");
    if (f) {
        char magic[4];
        size_t rc = fread(magic, 1, 3, f);
        fclose(f);
        if (rc == 3 && memcmp(magic, "PSF", 3) == 0) {
            return true;
        }
    }
...
std::set<std::string> HEPlugin::getSupportedExtensions() const
{
    return supported_ext;
}

std::vector<std::string> HEPlugin::getSecondaryFiles(const std::string& name)
{
    // .minipsf/.minipsf2 reference a shared .psflib/.psf2lib via PSF "_lib" tags;
    // fetch it alongside when streaming so the loader finds it on a clean machine.
