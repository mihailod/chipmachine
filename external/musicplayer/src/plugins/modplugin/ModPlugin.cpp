...
std::set<std::string> ModPlugin::getSupportedExtensions() const
{
...
ChipPlayer* ModPlugin::fromFile(const std::string& fileName)
{
    ...
    // Deflemask DMF is zlib-compressed; reject before modplug tries every parser.
    if (utils::path_extension(utils::toLower(fileName)) == "dmf" &&
        !data.empty() && data[0] == 0x78) {
