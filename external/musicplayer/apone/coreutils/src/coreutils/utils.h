...
// Implemented in apone/mods/coreutils/environment_apple.mm (Obj-C++).
// Forward-declared here so get_cache_dir() can call it without pulling
// Foundation.h into every .cpp TU that includes this header.
...
{
#ifdef __APPLE__
    // NSCachesDirectory = ~/Library/Caches/<appName>
    // Sandbox-legal. getenv("HOME") / ".cache" triggers the Documents
    // permission dialog because the sandbox intercepts all home traversal.
    auto d = fs::path(_mac_get_cache_subdir(appName));
#else
