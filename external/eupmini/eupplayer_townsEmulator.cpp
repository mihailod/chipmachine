...
// chipmachine integration: the standalone player linked SDL only for SDL_Delay()
// in the ring-buffer "wait for space" branch (and for its own audio callback).
// The plugin host drains the ring itself, so that branch is never taken; drop
// the SDL dependency here and provide a portable delay shim.
#include <thread>
#include <chrono>
        ...
        u_char const *pcmInst)
{
    ...
    if (instrument == nullptr) {
        // chipmachine: silenced — bank-less .eup files legitimately leave
        // instrument slots unset; this fired once per voice and spammed the host.
        return;
    }
        ...
        uint8_t const *pcmInst)
{
    ...
    if (instrument == nullptr) {
        // chipmachine: silenced (see TownsFmEmulator::setInstrumentParameter).
        return;
    }
